...
********************************************************************************
   ...
   Question:
     ...
     the fly, such as GetCgiEnvVar('HTTP_REFERER') ?

   Answer:
     It sometimes CLEANER and more READABLE in your code to use:

       webwriteln(CgiEnvVar.Referer);

     Compared to something ugly like this:

       webwriteln(GetCgiEnvVar('HTTP_REFERER'));
   
    You may also get single Cgi environment var's at a time by calling such
    a single procedure as:
      GetCgiUserAgent

    This is more performance but maybe more hassle and less code clarity than 
    using CgiEnvVars.UserAgent  
    
    As for putting them into a class or object?  Feel free to write a class
    if you want. 
...
begin
 ...
 {$IFDEF WIN32}
  ...
  SetLength(Result, windows.GetEnvironmentVariable(@Name[1], @Result[1], 8192));
 {$ENDIF}  
end;

{ These functions are one liners so I can count lines easily and match it to the records to make sure
  each one is implemented, it's easy to skip something when there are so many to implement.
  A good idea might be a source file automation tool that produced this file, since it is so
  repetitive. }
