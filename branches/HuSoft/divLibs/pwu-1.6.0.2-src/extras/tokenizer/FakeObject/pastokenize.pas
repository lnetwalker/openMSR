...
{ debug logging to a file.. does nothing if debug DEFINE switch is off }
procedure debuglog(msg: string);
begin
{$ifdef debug} // write to log file 
  writeln(debugt, msg);
{$endif}
end;
...
var
  i : integer;
begin
  if self = nil then begin debuglog('ppInitKeywords: nil self'); exit; end;
  if self^.privf.FKeywords = nil then 
  begin 
    debuglog('ppInitKeywords: nil Strlist'); 
    exit; 
  end;
  // add kewords from array into stringlist
...
function NewPasParser(aStrm: PChrStrm): PPasParser;
begin
  // must be a good stream or return nil
  if astrm = nil then begin result:= nil; exit; end;
  { create }
  new(Result);
  { setup record methods to point to unit procedures }
  result^.GetToken:= {$ifdef fpc}@{$endif}GetToken;
  result^.privf.FStrm := astrm;   // use character stream passed in
  result^.privf.FKeywords:= NewStrList;  //create the keywords list 
...
begin
  if p = nil then 
  begin
    debuglog('FreePasParser: nil param'); 
    exit;
  end;
  if p^.privf.FKeywords = nil then 
  begin
    debuglog('FreePasParser: nil FKeyWords'); 
    exit;
  end;
