                                     ...
                                     int stride)
{
    ...
    while (--height >= 0) {
        for (j = 0; j + 1 < width; j += 2) {
            ...
            *dest++ = CLIP(src[0] + u1); // if colors red and blue are wrong swap this lines
            *dest++ = CLIP(src[0] - rg);
            *dest++ = CLIP(src[0] + v1); // if colors red and blue are wrong swap this lines

            *dest++ = CLIP(src[2] + u1); // if colors red and blue are wrong swap this lines
            *dest++ = CLIP(src[2] - rg);
            *dest++ = CLIP(src[2] + v1); // if colors red and blue are wrong swap this lines
            src += 4;
        }
        src += stride - (width * 2);
    }
...
void Webcam::open_device(void)
{
      ...
      if (-1 == stat(device.c_str(), &st)) {
	    ...
	    throw runtime_error(device + ": cannot identify! " + ": " + strerror(errno));
      }
      ...
      if (-1 == fd) {
	    ...
	    throw runtime_error(device + ": cannot open! " + ": " + strerror(errno));
      }
}
