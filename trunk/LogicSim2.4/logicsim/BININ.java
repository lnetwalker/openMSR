...
public class BININ extends Gate{
  ...
  public void clicked(int mx, int my) {
    ...
    if (new Rectangle(x+5+5,y+5,15,15).contains(mx,my)) {
      if (displayType==0) value+=16;
      if (displayType==1) value+=10;
    } else if (new Rectangle(x+5+25,y+5,15,15).contains(mx,my)) {
      value+=1;
    } else if (new Rectangle(x+5+5,y+60,15,15).contains(mx,my)) {
      if (displayType==0) value-=16;
      if (displayType==1) value-=10;
    } else if (new Rectangle(x+5+25,y+60,15,15).contains(mx,my)) {
      value-=1;
    }

    if (value<0) value=0xff+value+1;
    if (value>0xff) value=value-0xff-1;
  ...
  public boolean showProperties(Component frame) {
    ...
    JDialog dlg=pane.createDialog(frame, I18N.getString("GATE_BINARYINPUT_PROPERTIES"));
