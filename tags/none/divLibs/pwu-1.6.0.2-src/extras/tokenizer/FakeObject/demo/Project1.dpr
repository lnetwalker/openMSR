{ Demo program showing how to parse Pascal Code and then do something with it..
  ...
  Pasdoc parsers or the FP compiler parser, and the Synedit parser.)
   ...
   This parser is similar to FastHTMLParser (search the net for it) but this is
  for Pascal parsing.

  Currently working on win32 and delphi. It will be easy get this running on
  linux and FPC by ifdeffing APPCONSOLE and adding $MODE OBJFPC.
...
begin
  ...
  NewInChrStrm(inputfile, {result}InChrStrm);
  NewPasParser(InChrStrm, {result} PasParser);
...
//    PasParser.Test;
//    InChrStrm.test;
    PasParser.GetToken(token, s, PasParser, InChrStrm);
...
begin
  ...
  writeln;
  writeln(output, 'please check output.txt for results');
