...
begin
  ...
  FOpened:= OpenFileRead(result^.privf.ftextcontent.afile, filename, 1);
  // return nil if File Opening went bad
  if FOpened = false then begin result:= nil; exit; end; 
...
//  if length(s) = 0 then s:= ''; // initialize incase
