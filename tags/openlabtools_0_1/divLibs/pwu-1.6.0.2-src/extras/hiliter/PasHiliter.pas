{ Routines to convert pascal file/string code to html highlighted file/string
 ...
 You can use a DLL or statically compile the hilighter into the code. I
 If not using DLL, then define STATIC_HILITER
}
...
uses
 {$ifdef STATIC_HILITER} 
  ChrStream, PasTokenize, {$else} DynPasTokenize,
 {$endif}
  compactsysutils,
  multitype,
//  pcharutils,
...
//      addtext(('<span class="InvalidToken">' + s + '</span>'), result);
    ...
    begin
      EscapeHTML(s); // could be < > & inside string
    ...
    begin
      EscapeHTML(s); // could be < > & inside comment
    ...
    case token of
      ...
      ptMultiply, ptAssign, ptOpenParen, ptCloseParen, ptOpenBracket, 
      ptCloseBracket, ptDivide:
        begin
          addtext(('<span class="Symbol">' + s + '</span>'), result);
        end;
      ptNotEquals, ptLess, ptLessEqual, ptGreater, ptGreaterEqual:
        begin
          EscapeHTML(s); // html entities - escape < >  
          addtext(('<span class="Symbol">' + s + '</span>'), result);        
        end;
    end;
