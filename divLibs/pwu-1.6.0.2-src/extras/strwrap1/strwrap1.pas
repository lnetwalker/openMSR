...
{--- public functions ---------------------------------------------------------}
  function OpenFileRead(var F:file; const Filename: string; recsize: integer): boolean;
  function OpenFileReWrite(var F:file; const Filename:string; recsize:integer): boolean;
  function OpenFile(var F:text; const Filename:string; mode: char): boolean; overload;
  function OpenFile(var F:file; const Filename:string; recsize:integer; mode: byte): boolean; overload;
  function OpenFile(var F: file; const filename: string; mode: char): boolean; overload;

  function MakeDir(s: string): boolean;

  function ReadChunk(const Filename: string; ChunkSize: integer; buf: pointer): boolean;
  function SaveChunk(filename: string; buf: pointer; chunksize: integer): boolean;

  function GetLineCount(const FileName: string): longint;
  function FindLine(const FileName: string; linenum: longword): boolean;  
  function GetFileSize(const FileName: string): longint;

  function StrLoadFile(const FileName: string): string;
  function StrSaveFile(const FileName, InputStr: string): boolean; overload;
  function StrSaveFile(const FileName, InputStr, endlnfeed: string): boolean; overload;
  function StrLoadLns(NumOfLines: integer; const FileName:string): String;
  function StrLoadRng(FromLine: integer; ToLine:integer; const FileName: string):String;

  function GetLn1(const FileName: string): string;
  function GetLn2(const FileName: string): string;
  function GetLnN(LineNumber: integer; const FileName: string): string;

  function ArrayLoadFile(const FileName:string): StrArray;
  function ArrayLoadFile_c(const FileName: string): StrArray_c;
  function ArrayLoadFile_0(const FileName: string): StrArray;
  function ArrayLoadFile_c_0(const FileName: string): StrArray_c;
  function ArrayLoadLns(NumOfLines: integer; const FileName: string): StrArray;
  function ArrayLoadLns_0(NumOfLines: integer; const FileName: string): StrArray;
  function ArrayLoadRng(FromLine: integer; ToLine:integer; const FileName: string): StrArray;
  function ArrayLoadRng_0(FromLine: integer; ToLine:integer; const FileName: string): StrArray;

  function GetLastLns(const FileName: string; NumLines: longint): string;

  function RemoveDupStrings(var InputStrArray: StrArray): integer;
(*  TO DO

  function StrLoadExact(LineNumsArray: LnArray; FileName: string): string;
  function ArrayLoadExact(LineNumsArray: LnArray; FileName: string): StrArray;
  function ArrayLoadExact_0(LineNumsArray: LnArray; FileName: string): StrArray;
  function ArraySaveFile(InputArray: StrArray; FileName: string): boolean;
...
uses
  {$IFDEF WIN32}windows{$ELSE}baseunix{$ENDIF};
...
function OpenFile(var F:text; const Filename:string; mode: char): boolean;
var
  oldFM: byte;
begin
...
function OpenFile(var F:file; const Filename:string; recsize:integer; mode: byte): boolean;
var                                                                                        
  oldFM: byte;
begin
  oldFM:= filemode;
...
function OpenFile(var F:file; const filename: string; mode:char): boolean; 
var
  oldFM: byte;
begin
  if ( mode in ['R', 'W'] ) then
...
var
  ...
  hInfo: THANDLE;
begin
  hInfo:= FindFirstFile(pChar(FileName),
          {$ifdef fpc} @FileInfo
          {$else}       FileInfo {$endif});
...
begin
  Result:=-1;
  if not OpenFile(F, FileName, 'r') then EXIT;
  Result:=0;
  while not EOF(F) do
  begin
    Readln(F);
...
begin
  result:= ''; // init               
  if OpenFile(F, Filename, 1, fmOpenReadWrite) = false then 
  begin 
    result:= '-1NF';  EXIT;
  end;
...
begin
  result:= false;
  if OpenFileRewrite(F, Filename, 1) = false then EXIT; // open in write mode
...
begin
  result:= false;
  if OpenFileRead(F, Filename, 1) = false then EXIT; // open in write mode
...
function StrLoadLns(NumOfLines: longint; const FileName: string): string;
var
  F: text;
  str1: string;
  Line: string;
...
begin
  result:= ''; //safety
  if not OpenFile(F, FileName, 'r') then
...
function StrLoadRng(FromLine:longint; ToLine:longint; const FileName: string): string;
var
  F: text;
  str1: string;
  Line: string;
...
function GetLnN(LineNumber: longint; const FileName: string): string;
var
  F: text;
...
{ Load the first N lines of a file into an array of strings. Each line becomes
  ...
  Note: the array contents start at array[1], since we deal with line 1, not
  line 0, for clarity. }
function ArrayLoadLns(NumOfLines:longint; const FileName: string): StrArray;
var
  F: text;
  Line: string;
  iLoc:longint;
...
function ArrayLoadLns_0(NumOfLines:longint; const FileName: string): StrArray;
var
  F: text;
  Line: string;
  iLoc:longint;
...
function ArrayLoadRng(FromLine:longint; ToLine:longint; const FileName: string): StrArray;
var
  F: text;
  Line: string;
  iLoc:longint;
...
function ArrayLoadRng_0(FromLine:longint; ToLine:longint; const FileName: string): StrArray;
var
  F: text;
  Line: string;
  iLoc,
...
function Array0LoadExact(LineNumsArray:LnArray; FileName:string): StrArray;
var
  F: text;
  Line: string;
  iLoc,
...
// Save a string directly to a file
{ function StrSaveFile(InputString:string; FileName: string): boolean; }
