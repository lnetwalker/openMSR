...
function ColumnIndex(cp: SDS_ColumnInfo; const name: string): longword;
function ColumnInfo(const FileName: string): SDS_ColumnInfo;
function ColumnName(cp: SDS_ColumnInfo; index: longword): string;
function ColumnType(cp: SDS_ColumnInfo; index: longword): byte;
...
function FetchColumn(ap: SDS_Array; index: longword): string;
function FetchColumn_float(ap: SDS_Array; index: longword): double;
function FetchColumn_int(ap: SDS_Array; index: longword): longint;
...
function ImportSQL(const dump: string): longword;
function LastID(const FileName: string): longword;
function NumFields(const FileName: string): longword;
function NumRows(const FileName: string): longword;
function ResultCmd(rp: SDS_Result): string;
function ResultEOF(rp: SDS_Result): boolean;
function ResultError(rp: SDS_Result): string;
function ResultFields(rp: SDS_Result): longword;
function ResultPointer(rp: SDS_Result): longword;
procedure ResultRewind(rp: SDS_Result);
function ResultRows(rp: SDS_Result): longword;
procedure ResultSeek(rp: SDS_Result; index: longword);
function ResultTime(rp: SDS_Result): double;
function TotalFields(const FileName: string): longword;
...
procedure RowsetSet(var res: SDS_PRowSet; index: longword; row: SDS_PRow);
var
  i: longword;
begin
...
procedure ShellSort(var rowset: array of SDS_PRow; key: longword; cs: SDS_PCols);
  type
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        while (j >= 0) and (substrings.strcomp(rowset[j]^[key], temp^[key]) > 0) do
        begin
...
function ReadRow(var fh: text; fields: longword): SDS_Row;
var 
  cnt: longword;
begin
...
function ReadCols(var fh: text; fields: longword): SDS_Cols;
var 
  i: longword;
begin
...
begin
  ...
  begin
    ...
    begin
      // Greater than
      case t of
          1: if substrings.strcomp(row^[cond^[i].field], cond^[i].value) > 0 then sub := true;
          2: if n > nc then sub := true;
          3: if f > fc then sub := true;
          4: if DateCompare(row^[cond^[i].field], cond^[i].value) > 0 then sub := true;
          5: if TimeCompare(row^[cond^[i].field], cond^[i].value) > 0 then sub := true;
    ...
    begin
      ...
      if substrings.stricomp(row^[cond^[i].field], cond^[i].value) = 0 then sub := true;
    end;
...
begin
  result := 0;
  if not FileExists_plain(FileName) then exit;
  FileMarkRead(FileName, fk);
  assign(fh, FileName);
  reset(fh);
  hdr := ReadHeader(fh);
...
function InsertRow(const FileName: string; fields: SDS_PFieldList; values: SDS_PRow): longword;
var          
  fh, fhn: text;
  buff: string;
  hdr: SDS_Header;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        case cols[fields^[0]].data_type of
            1: if substrings.strcomp(row[fields^[0]], st) <= 0 then continue else st := row[fields^[0]];
            2: begin
...
// SQL update
function Update(const FileName: string; updset: SDS_PSet; cond: SDS_PCondition): longword;
var fh, fhn: text;
    i: longword;
    row: SDS_Row;
    hdr: SDS_Header;
...
function DeleteRow(const FileName: string; cond: SDS_PCondition): longword;
var
  fh, fhn: text;
  row: SDS_Row;
  hdr: SDS_Header;
