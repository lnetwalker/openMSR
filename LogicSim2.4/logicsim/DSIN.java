...
/**
 * Title:        LogicSim
 * Module:	 DSIN.java ( based on BININ.java )
...
/* Binary input: wie LCD, aber mit Einstellung */

public class DSIN extends Gate{
  ...
  public void simulate() {
    ...
    in.close();
      ...
      System.err.println("Error reading from DeviceServer");
    }
    /*
      a request like :
    ...
    */
    ...
    // remove html tags
    nohtml = res.replaceAll("\\<.*?>","");
    
    // remove spaces
    nohtml = nohtml.replaceAll(" ","");
    
    // no walk over the remaing string and set the values
    // of the out array
    if ( nohtml.length() != 0 )
      for (int i=0; i<8; i++) {
	if ( nohtml.substring( i, i+1 ).equals("1")) {
	  out[i]=true;
	} else {
	  out[i]=false;
	}  
  ...
  public boolean showProperties(Component frame) {
    ...
    if ( DSIN_URL.length()==0 )
      DSIN_URL="http://";

    DSIN_URL = (String)JOptionPane.showInputDialog(frame, I18N.getString("MESSAGE_DSIN_READURL"), "Device Server URL",
                                                   JOptionPane.QUESTION_MESSAGE, null,null, DSIN_URL);
    return true;
  }
